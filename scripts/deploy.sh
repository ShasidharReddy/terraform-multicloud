...
prompt_action() {
  local input i
  while true; do
    log "${BOLD}Select action${RESET}"
    for ((i=0; i<${#ACTIONS[@]}; i++)); do
      log "  $((i + 1))) ${ACTIONS[$i]}"
    done
    printf "%b" "${YELLOW}Choose one action:${RESET} "
    read -r input
    [[ "$input" =~ ^[0-9]+$ ]] || {
      log "${RED}Invalid action.${RESET}"
      continue
    }
    [ "$input" -ge 1 ] && [ "$input" -le "${#ACTIONS[@]}" ] || {
      log "${RED}Invalid action.${RESET}"
      continue
    }
    ACTION="${ACTIONS[$((input - 1))]}"
    break
  done
}
...
run_terraform() {
  ...
  (
    ...
    case "$ACTION" in
      plan)
        terraform plan -input=false -var="vm_count=$vm_count"
        ;;
      apply)
        terraform apply -input=false -var="vm_count=$vm_count" -auto-approve
        ;;
      destroy)
